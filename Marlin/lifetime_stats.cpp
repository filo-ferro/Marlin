...
#define LIFETIME_MAGIC 0x24216763
...
void lifetime_stats_tick()
{
    ...
    if (minute_counter_millis < m)
    {
        ...
        if (is_printing)
        {
            ...
            lifetime_stats_update_e(current_position[E_AXIS]);
        }
...
static void save_lifetime_stats()
{
    ...
    eeprom_write_dword((uint32_t*)(LIFETIME_EEPROM_OFFSET + 28), last_print_minutes);
    eeprom_write_dword((uint32_t*)(LIFETIME_EEPROM_OFFSET + 32), last_print_centimeters);
}
...
void print_lifetime_stats(int condensed)
{
	...
	SERIAL_ECHOPAIR(":", lifetime_minutes);
        SERIAL_ECHOPAIR("|", lifetime_print_minutes);
        SERIAL_ECHOPAIR("|", lifetime_print_centimeters);
        SERIAL_ECHOPAIR(":", triptime_minutes);
        SERIAL_ECHOPAIR("|", triptime_print_minutes);
        SERIAL_ECHOPAIR("|", triptime_print_centimeters);
        SERIAL_ECHOPAIR(":", last_print_minutes);
        SERIAL_ECHOPAIR("|", last_print_centimeters);
        SERIAL_ECHOLN("");
    } else {
        SERIAL_ECHO_START;
